...
>
> "Servo ถูกใช้ทั่วไปเลย — ตั้งแต่เครื่องบินบังคับ, หุ่นยนต์, กล้องวงจรปิด, และแม้แต่เครื่องพิมพ์ 3D! ในบทนี้เราจะมาทำความรู้จัก Servo SG90 กัน แล้วเอามาทำโปรเจกต์สุดเจ๋งอย่าง 'เข็มขั้นบันไดอัตโนมัติ' ที่สาขึ้นลงได้เองเลย!"
...
| สายจัมเปอร์ | หลายเส้น | ผู้อ่านแนะนำใช้สายสีแยกเพื่อความปลอดภัย |
...
```
    ...
    │   GPIO10├──────────────│○ สีส้ม    │ ← PWM Signal (ใช้ GPIO 10 แทน GPIO 2 ซึ่งเป็น Strapping Pin)
...
```
    ESP32-C3        Servo SG90        แหล่งจ่ายไฟแยก
    ┌─────────┐   ┌──────────┐     ┌──────────┐
    │      GND├───┤○ น้ำตาล  │◄────│  GND     │  ← GND ร่วม
    │  GPIO10 ├───┤○ สีส้ม   │     └──────────┘
    └─────────┘   │○ แดง    │◄────│  5V 1A+  │  ← USB Adapter 5V (แยก!)
                   └──────────┘     └──────────┘
...
> ⚠️ **สำคัญ:** สายแดง (VCC) ของ Servo ต้องต่อกับ **USB Adapter 5V** เท่านั้น! ห้ามต่อเข้ากับขา 5V ของ ESP32-C3 เพราะ Servo ดึงกระแสสูงมาก!
...
// ==== กำหนดขา Pin ====
#define SERVO_PIN    10  // Servo Signal → GPIO 10 (GPIO 2 เป็น Strapping Pin)
...
void setup() {
  ...
  myServo.attach(SERVO_PIN);      // ต่อ Servo ที่ GPIO 10
...
```
    ...
    │  GPIO 10 ──► Servo Signal (ส้ม)           │
    │  GPIO 18 ──► LED เขียว (ผ่าน R 220Ω)      │
    │  GPIO 19 ──► LED แดง (ผ่าน R 220Ω)       │
    └──────────────────────────────────────────┘
...
// ==== กำหนดขา Pin ====
#define SERVO_PIN      10  // Servo Signal → GPIO 10
#define PIR_PIN        4   // PIR Sensor OUT → GPIO 4
#define LED_GREEN_PIN  18  // LED เขียว (เปิด) → GPIO 18
#define LED_RED_PIN    19  // LED แดง (รอ) → GPIO 19
...
```cpp
// ==== กำหนดขา ====
#define SERVO_PIN   10  // (GPIO 2 เป็น Strapping Pin)
...
void IRAM_ATTR emergencyStop() {
  ...
  // หมายเหตุ: LED_BUILTIN ไม่ได้ defined บน ESP32-C3
  // ควรใช้ digitalWrite(PIN ที่กำหนด, LOW) แทน
}
